...
void CameraV4L2::setCameraParams()
{
    ...
    dev << v4l2_control{ V4L2_CID_EXPOSURE_AUTO, V4L2_EXPOSURE_MANUAL }
        << v4l2_control{ V4L2_CID_EXPOSURE_ABSOLUTE, 15 }
...
void Tracker::MainLoop()
{
    ...
    while (mainThreadRunning && camera->isRunning())
    {
        ...
        for (int i = 0; i < trackerNum; i++)
        {
            ...
            int searchLeft   = static_cast<int>(trackerStatus[i].oldCenter.x - 5*trackerStatus[i].searchSize);
            int searchRight  = static_cast<int>(trackerStatus[i].oldCenter.x + 5*trackerStatus[i].searchSize);
            int searchTop    = static_cast<int>(trackerStatus[i].oldCenter.y - 5*trackerStatus[i].searchSize);
            int searchBottom = static_cast<int>(trackerStatus[i].oldCenter.y + 5*trackerStatus[i].searchSize);
        ...
        for (int i = 0; i < trackerNum; ++i) {
            ...
            try
            {
                ...
                if (ids.size() == 0 || cv::aruco::estimatePoseBoard(corners, ids, trackers[connection->connectedTrackers[i].TrackerId], parameters->camMat, parameters->distCoeffs, trackerStatus[i].boardRvec, trackerStatus[i].boardTvec, trackerStatus[i].boardFound && parameters->usePredictive) <= 1)
                {
                    for (int j = 0; j < 6; j++)
                    {
