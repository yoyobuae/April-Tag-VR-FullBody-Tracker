...
void Tracker::MainLoop()
{
...

    {
        ...
        frameCount = 0;
        calibratorProjected.clear();
        calibratorReprojected.clear();
        calibratorPoints.clear();
    }
    ...
    while (mainThreadRunning && cameraRunning)
    {
        ...
        for (int i = 0; i < trackerNum; i++)
        {
            ...
            cv::projectPoints(point, identity_rvec, identity_tvec, parameters->camMat, parameters->distCoeffs, projected1);
            ...
            if (trackerStatus[i].boardFound)
            {
                ...
                {
                    calibratorProjected.push_back(projected1[1]);

                    cv::Mat rpos = (cv::Mat_<double>(4, 1) << point[1].x, point[1].y, point[1].z, 1);
                    rpos = wtranslation * rpos;

                    cv::Point3d curPoint;

                    curPoint.x = rpos.at<double>(0,0);
                    curPoint.y = rpos.at<double>(1,0);
                    curPoint.z = rpos.at<double>(2,0);

                    cv::Point3d delta = curPoint - prevPoint;
                    prevPoint = curPoint;

                    double distance = sqrt(delta.x * delta.x + delta.y * delta.y + delta.z * delta.z);
                    speed = speed * 0.9 + distance * 0.1;

                    bool startResting = false;
                    bool stopResting = false;

                    if ((speed < 0.003) && (!isResting)) {
                        startResting = true;
                    }

                    if ((speed > 0.009) && (isResting)) {
                        stopResting = true;
                    }
                    ...
                    if (stopResting) {
                        ...
                        if (avgPoints.size() > 15) {
                            avgPoints.resize(avgPoints.size() - 15);

                            cv::Point3d avg = cv::Point3d(0, 0, 0);

                            for (auto p : avgPoints) {
                                avg.x += p.x;
                                avg.y += p.y;
                                avg.z += p.z;
                            }

                            avg.x /= avgPoints.size();
                            avg.y /= avgPoints.size();
                            avg.z /= avgPoints.size();

                            calibratorPoints.push_back(avg);
                            calibratorTimes.push_back(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count());

                            if (calibratorPoints.size() > pointsThreshold)
                            {

                                double avgY = 0;
                                printf("start points dump ***\n");
                                for (auto p : calibratorPoints) {
                                    printf("%3.3f\t%3.3f\t%3.3f\n", p.x, p.y, p.z);
                                    avgY += p.y;
                                }
                                printf("end points dump ***\n");

                                avgY /= calibratorPoints.size();

                                std::vector<cv::Point3d> flat;
                                for (auto p : calibratorPoints) {
                                    flat.push_back(cv::Point3d(p.x, avgY, p.z));
                                }

                                cv::Mat wtranslation1 = transformFromPoints(calibratorPoints, flat);
                                wtranslation = wtranslation1 * wtranslation;
                                cv::Mat R = (cv::Mat_<double>(3, 3) <<
                                             wtranslation.at<double>(0, 0), wtranslation.at<double>(0, 1), wtranslation.at<double>(0, 2),
                                             wtranslation.at<double>(1, 0), wtranslation.at<double>(1, 1), wtranslation.at<double>(1, 2),
                                             wtranslation.at<double>(2, 0), wtranslation.at<double>(2, 1), wtranslation.at<double>(2, 2));
                                wrotation =  mRot2Quat(R);
                                ...
                                parameters->wrotation = wrotation;
                                parameters->wtranslation = wtranslation;
                                parameters->calibOffsetX = 100 * wtranslation.at<double>(0, 3);
                                parameters->calibOffsetY = 100 * wtranslation.at<double>(1, 3);
                                parameters->calibOffsetZ = 100 * wtranslation.at<double>(2, 3);
                                parameters->calibOffsetA = -(180.0/M_PI) * eulerAngles.z;
                                parameters->calibOffsetB = -(180.0/M_PI) * eulerAngles.x;
                                parameters->calibOffsetC = (180.0/M_PI) * eulerAngles.y;
                                parameters->Save();

                                calibratorProjected.clear();
                                calibratorReprojected.clear();
                                calibratorPoints.clear();
                                calibratorTimes.clear();

                                pointsThreshold += pointsThresholdIncrement;
                                pointsThresholdIncrement += 1;
                            }
                        }
                    }

                    if (isResting) {
                        avgPoints.push_back(curPoint);

                    }
                }
            }

            if (calibratorProjected.size() > 0)
            {
                for (int i = 0; i < calibratorProjected.size(); ++i)
                {
                    if (i % 15 != 0)
                        continue;
                    const auto& position = calibratorProjected[i];
                    cv::circle(drawImg, position, 5, cv::Scalar(0, 255, 255), 2, 8, 0);
                    cv::circle(drawImgMasked, position, 5, cv::Scalar(0, 255, 255), 2, 8, 0);
                }
            }

            if (calibratorReprojected.size() > 0)
            {
                for (int i = 0; i < calibratorReprojected.size(); ++i)
                {
                    if (i % 15 != 0)
                        continue;
                    const auto& position = calibratorReprojected[i];
        ...
        int size = ((gray.rows > gray.cols) ? gray.rows : gray.cols) * parameters->searchWindow;
        ...
        for (int i = 0; i < trackerNum; ++i) {
            ...
            try
            {
                ...
                if (cv::aruco::estimatePoseBoard(corners, ids, trackers[i], parameters->camMat, parameters->distCoeffs, trackerStatus[i].boardRvec, trackerStatus[i].boardTvec, trackerStatus[i].boardFound && parameters->usePredictive) <= 0)
                {
                    for (int j = 0; j < 6; j++)
                    {
